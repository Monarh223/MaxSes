...
async def cmd_work(message: Message):
    ...
    if not c.fetchone():
        ...
        conn.commit()
        conn.close()
        await message.answer("✅ Группа добавлена и активирована!\n/esim")
        return
    ...
    conn.commit()
    conn.close()
    await message.answer("✅ Бот активирован!" if not cur else "⏸️ Бот отключён.")
...
async def group_request(callback: CallbackQuery):
    ...
    if op[active_field] != 1:
        await callback.answer("❌ Режим отключён"); return
    channels = get_active_channels()
    if not channels:
        await callback.answer("Нет каналов"); return
    ...
    try:
        ...
        conn.commit()
        conn.close()
        await safe_edit_text(callback.message, f"<b>✅ ЗАЯВКА #{order_id} СОЗДАНА</b>\n\n📱 {op['emoji']} {op['name']} · {price}$ · {mode}\n\n<i>Ожидайте исполнителя...</i>")
...
async def save_esim(message: Message, state: FSMContext, file_id: str, phone: str, order_id: int = None):
    ...
    if order_id:
        ...
        c.execute('UPDATE users SET qr_month = qr_month + 1, total_qr = total_qr + 1, pending_balance = pending_balance + ? WHERE user_id = ?', (order['price'], user_id))
        c.execute('INSERT INTO phone_submissions (phone, user_id, order_id, submitted) VALUES (?, ?, ?, ?)', (phone, user_id, order_id, moscow_time().isoformat()))
        conn.commit()
...
async def order_status_action(callback: CallbackQuery):
    ...
    order = c.fetchone()
    if not order:
        await callback.answer("Не найден"); conn.close(); return
    executor_id = order['executor_id']

    if action == "up":
        c.execute('UPDATE orders SET credited = 1, blocked = 0, noscan = 0 WHERE id = ?', (order_id,))
        c.execute('UPDATE users SET pending_balance = pending_balance - ?, expected_balance = expected_balance + ? WHERE user_id = ?', (order['price'], order['price'], executor_id))
        conn.commit()
        conn.close()
...
async def admin_operators(callback: CallbackQuery):
    ...
    text = "<b>💰 ОПЕРАТОРЫ</b>\n\n"
    for op in ops:
        bh_status = "🟢" if op['active_bh'] else "🔴"
        hd_status = "🟢" if op['active_hd'] else "🔴"
        text += f"{op['emoji']} <b>{op['name']}</b>\nБХ: {bh_status} {op['price_bh']}$ | ХД: {hd_status} {op['price_hd']}$\n\n"
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def op_emoji_received(message: Message, state: FSMContext):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute('INSERT OR REPLACE INTO operators (name, price_bh, price_hd, emoji) VALUES (?,?,?,?)', (data['op_name'], data['op_bh'], data['op_hd'], emoji))
    conn.commit()
    conn.close()
...
async def admin_toggle_bh(callback: CallbackQuery):
    ...
    ops = get_operators()
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"{'🟢' if op['active_bh'] else '🔴'} БХ · {op['emoji']} {op['name']}", callback_data=f"tbh_{op['id']}")] for op in ops
    ] + [[InlineKeyboardButton(text="🔙 Назад", callback_data="admin_operators")]])
...
async def admin_toggle_hd(callback: CallbackQuery):
    ...
    ops = get_operators()
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"{'🟢' if op['active_hd'] else '🔴'} ХД · {op['emoji']} {op['name']}", callback_data=f"thd_{op['id']}")] for op in ops
    ] + [[InlineKeyboardButton(text="🔙 Назад", callback_data="admin_operators")]])
...
async def admin_channels(callback: CallbackQuery):
    ...
    channels = get_active_channels()
    text = "<b>📢 КАНАЛЫ</b>\n\n"
    for ch in channels: text += f"• {ch['username'] or ch['channel_id']}\n"
    if not channels: text += "<i>Нет</i>"
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="➕", callback_data="admin_add_channel"),
         InlineKeyboardButton(text="🗑️", callback_data="admin_del_channel")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="admin_back")],
    ])
    await safe_edit_text(callback.message, text, reply_markup=kb)
    await callback.answer()
...
async def admin_groups(callback: CallbackQuery):
    ...
    conn.close()
    text = "<b>👥 ГРУППЫ</b>\n\n"
    for g in groups: text += f"{'🟢' if g['active'] else '🔴'} {g['username'] or g['group_id']}\n"
...
async def admin_users(callback: CallbackQuery):
    ...
    conn.close()
    text = "<b>👤 УЧАСТНИКИ</b>\n\n"
    for u in users: text += f"• @{u['username'] or u['user_id']} | {u['rank']} | QR:{u['total_qr']} | ${u['balance']}\n"
    await safe_edit_text(callback.message, text, reply_markup=back_to_admin())
    await callback.answer()
...
async def pay_user_received(message: Message, state: FSMContext):
    ...
    conn = get_db()
    c = conn.cursor()
    if target.isdigit(): c.execute('SELECT * FROM users WHERE user_id = ?', (int(target),))
    else: c.execute('SELECT * FROM users WHERE username = ?', (target,))
    user = c.fetchone()
    conn.close()
    if not user:
...
async def pay_amount_received(message: Message, state: FSMContext):
    ...
    if amount > expected: await message.answer(f"❌ Макс: {expected}$"); return
    conn = get_db()
    c = conn.cursor()
...
async def admin_order_do(callback: CallbackQuery):
    ...
    op = c.fetchone()
    channels = get_active_channels()
    if not channels: await callback.answer("Нет каналов"); conn.close(); return
    active_field = 'active_bh' if mode == 'БХ' else 'active_hd'
    price = op['price_bh'] if mode == 'БХ' else op['price_hd']
    if op[active_field] != 1: await callback.answer("❌ Режим отключён"); conn.close(); return
...
async def broadcast_send(message: Message, state: FSMContext):
    ...
    cnt = 0
    for u in users:
        try: await bot.send_message(u['user_id'], message.text); cnt += 1; await asyncio.sleep(0.05)
        except: pass
...
async def db_file_received(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id): return
    if not message.document.file_name.endswith('.db'): await message.answer("❌ .db!"); return
    try:
...
async def profile(callback: CallbackQuery):
    ...
    if not user: ensure_user(callback.from_user.id, callback.from_user.username, callback.from_user.first_name); user = get_user(callback.from_user.id)
    text = (f"<b>👤 ПРОФИЛЬ</b>\n\n"
...
async def balance_history(callback: CallbackQuery):
    ...
    if rows:
        for r in rows: text += f"• {r['type']}: {'+' if r['amount']>=0 else ''}{r['amount']}$ ({r['created'][:10]})\n"
    else: text += "<i>Нет операций</i>"
...
async def withdraw_amount(callback: CallbackQuery):
    ...
    user = get_user(callback.from_user.id)
    if user['balance'] < amount: await callback.answer("Недостаточно"); return
    conn = get_db()
    c = conn.cursor()
    ...
    if channels:
        ...
        try: await bot.send_message(chat_id=channels[0]['channel_id'], text=msg, reply_markup=kb)
        except: pass
...
async def operators_list(callback: CallbackQuery):
    ops = get_operators()
    text = "<b>📊 ЦЕНЫ БХ/ХД</b>\n\n"
    for op in ops: text += f"{'✅' if op['active_bh'] or op['active_hd'] else '❌'} {op['emoji']} <b>{op['name']}</b> · {op['price_bh']}$/{op['price_hd']}$\n"
...
async def help_cmd(callback: CallbackQuery):
    await safe_edit_text(callback.message, "<b>ℹ️ ПОМОЩЬ</b>\n\n/esim — запрос номера\nБХ/ХД — режимы\n5 мин на сдачу\n3 предупреждения = блок 1ч", reply_markup=back_to_main())
    await callback.answer()

@dp.callback_query(F.data == "back_main")
async def back_main(callback: CallbackQuery):
    await safe_edit_text(callback.message, "<b>🚀 ERWINS ESIM BOT</b>\n\nВыберите действие:", reply_markup=main_menu())
    await callback.answer()

@dp.callback_query(F.data == "admin_back")
async def admin_back(callback: CallbackQuery):
    if not is_admin(callback.from_user.id): return
    await safe_edit_text(callback.message, "<b>🛠️ АДМИН-ПАНЕЛЬ</b>", reply_markup=admin_menu())
    await callback.answer()
