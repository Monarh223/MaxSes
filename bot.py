...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS orders (
        ...
        hold_until TEXT, paid INTEGER DEFAULT 0)''')
...
async def cmd_esim(message: Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[])
    for op in ops:
        kb.inline_keyboard.append([InlineKeyboardButton(text=f"{op['emoji']} {op['name']} · {op['price']}$", callback_data=f"group_req_{op['id']}")])
...
async def group_request(callback: CallbackQuery):
    ...
    c.execute('INSERT INTO orders (operator, price, mode, status, group_id, requester_id) VALUES (?, ?, ?, ?, ?, ?)',
              (op['name'], op['price'], get_setting('default_mode'), 'active', group_id, requester_id))
...
async def save_esim(message: Message, state: FSMContext, file_id: str, phone: str, order_id: int = None):
    ...
    if order_id:
        ...
        # Отправка в группу кто запросил
...
async def order_status_action(callback: CallbackQuery):
    ...
    if action == "up":
        ...
        if current_mode == 'БХ':
            ...
            await callback.answer("Нажмите Оплатить для выплаты")
        else:
            # Холд - будет обработан автоматически
            await callback.message.edit_caption(
                caption=callback.message.caption + f"\n\n⏳ <b>Холд до:</b> {order['hold_until'][:19] if order['hold_until'] else 'Н/Д'}",
                reply_markup=None
            )
            await callback.answer("На холде. Выплата после истечения холда.")
        ...
        c.execute('UPDATE orders SET status = ? WHERE id = ?', ('noscan', order_id))
        conn.commit()
...
async def admin_channels(callback: CallbackQuery):
    ...
    channels = get_active_channels()
    text = "<b>📢 КАНАЛЫ</b>\n\n" + ("\n".join([f"• {ch['username'] or ch['channel_id']}" for ch in channels]) if channels else "<i>Нет каналов</i>")
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def toggle_operator(callback: CallbackQuery):
    ...
    row = c.fetchone()
    c.execute('UPDATE operators SET active = ? WHERE id = ?', (0 if row['active'] else 1, op_id))
    conn.commit()
    conn.close()
...
async def admin_mode(callback: CallbackQuery):
    ...
    await callback.message.edit_text(f"<b>🎯 РЕЖИМ: {current}</b>", reply_markup=kb)
    await callback.answer()
...
async def admin_db_export(callback: CallbackQuery):
    if not is_admin(callback.from_user.id): return
    try:
        await callback.message.answer_document(FSInputFile(DB_PATH), caption=f"📦 {datetime.now().strftime('%Y-%m-%d %H:%M')}")
        await callback.answer("✅")
    except Exception as e:
        await callback.answer(f"Ошибка: {e}")
...
async def admin_create_order(callback: CallbackQuery):
    ...
    ops = get_operators()
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"{op['emoji']} {op['name']} · {op['price']}$", callback_data=f"order_op_{op['id']}")] for op in ops
    ] + [[InlineKeyboardButton(text="🔙 Назад", callback_data="admin_back")]])
    await callback.message.edit_text("<b>📱 ВЫБЕРИТЕ ОПЕРАТОРА</b>", reply_markup=kb)
    await callback.answer()
...
async def admin_order_created(callback: CallbackQuery):
    ...
    c.execute('INSERT INTO orders (operator, price, mode, status) VALUES (?, ?, ?, ?)', (op['name'], op['price'], get_setting('default_mode'), 'active'))
    order_id = c.lastrowid
    conn.commit()
    conn.close()
...
async def admin_users(callback: CallbackQuery):
    ...
    text = "<b>👤 УЧАСТНИКИ</b>\n\n"
    for u in users:
        text += f"• @{u['username'] or u['user_id']} | {u['rank']} | {u['total_qr']} QR\n"
    await callback.message.edit_text(text, reply_markup=back_to_admin())
    await callback.answer()
...
async def help_cmd(callback: CallbackQuery):
    text = "<b>ℹ️ ПОМОЩЬ</b>\n\n<b>Сдать ESIM:</b> нажмите «Сдать ESIM» в ЛС бота\n<b>Взять заказ:</b> нажмите кнопку в канале или /esim в группе\n\n<b>Группа:</b> /work, /esim, /orders, /rating"
    await callback.message.edit_text(text, reply_markup=back_to_main())
    await callback.answer()
